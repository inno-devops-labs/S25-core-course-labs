...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _init_client(self):
        ...
        if file is not None:
            ...
            sdk = yandexcloud.SDK(sa_key=token)

        elif self.get_option("yacloud_token"):
            # oauth token
            token = self.get_option("yacloud_token")
            sdk = yandexcloud.SDK(token=token)

        elif environ.get("YC_TOKEN"):
            token = environ.get("YC_TOKEN")
            sdk = yandexcloud.SDK(iam_token=token)
            print("iam token used")
        else:
            raise AnsibleError(
                "token it empty. provide either `yacloud_token_file` or `yacloud_token` or use `YC_TOKEN` environment variable"
            )
