    ...
    def verify_file(self, path):
        if super(InventoryModule, self).verify_file(path):
            if path.endswith(('yacloud_compute.yml', 'yacloud_compute.yaml')):
                return True
        display.debug("yacloud_compute inventory filename must end with 'yacloud_compute.yml' or 'yacloud_compute.yml'")
        return False
        ...
        for instance in self.hosts:
            ...
            if instance["status"] == "RUNNING":
                ...
                if ip:
                    ...
                    self.inventory.add_host(host=host_name, group=group)

                    self.inventory.set_variable(host_name, 'ansible_user', 'catorleader')
                    self.inventory.set_variable(host_name, 'ansible_ssh_private_key_file', '~/.ssh/id_ed25519')
