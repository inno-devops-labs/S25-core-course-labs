...
I have implemented by setting the pod management policy to `Parallel` in statefulset.yaml.

## Bonus task

I have completed all of these steps for the bonus app, and added the Rolling Update strategy.

Here is a couple of features of OnDelete:

- Behavior: Pods do not update automatically when the StatefulSet definition changes. Instead, each pod must be deleted manually for the update to take effect.
- Use Case:
  1. When manual control is needed for updates (e.g., handling database upgrades where downtime needs to be carefully managed).
  2. When pods have critical stateful data that must be migrated carefully.
  3. Ideal for applications requiring controlled upgrades with manual intervention.

And for RollingUpdate:

- Behavior: Updates pods one at a time, in reverse order (highest ordinal first). Ensures that at least one instance remains available.
- Use Case:
  1. For stateful applications that can tolerate updates without manual intervention.
  2. Common for distributed databases, queues, and other clustered applications where automatic rolling updates are safe.
  3. Ensures zero downtime if the application supports staggered restarts.