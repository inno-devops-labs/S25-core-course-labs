...
There is a demonstration of DNS resolution:

```sh
(venv) vm@vm /m/v/d/h/V/U/L/P/D/S/k8s (lab14)> kubectl get svc
NAME                       TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)             AGE
kubernetes                 ClusterIP   10.96.0.1        <none>        443/TCP             22h
python-app                 ClusterIP   10.104.144.107   <none>        5000/TCP            28s
python-app-headless        ClusterIP   None             <none>        5000/TCP            28s
vault                      ClusterIP   10.96.124.72     <none>        8200/TCP,8201/TCP   20h
vault-agent-injector-svc   ClusterIP   10.103.138.23    <none>        443/TCP             20h
vault-internal             ClusterIP   None             <none>        8200/TCP,8201/TCP   20h
(venv) vm@vm /m/v/d/h/V/U/L/P/D/S/k8s (lab14)> kubectl exec python-app-0 -- nslookup python-app-1.python-app-headless.default.svc.cluster.local
Defaulted container "python-app" out of: python-app, vault-agent, vault-agent-init (init)
;; Got recursion not available from 10.96.0.10
Server:		10.96.0.10
Address:	10.96.0.10#53

Name:	python-app-1.python-app-headless.default.svc.cluster.local
...
We do not need to add liveness/readiness probes, since they are there by default in helm-created manifest. As I wrote above, they are the reason of counter incrementing. Probes realized as requests or connections (in our case they are just HTTP GET requests to the root). `liveness` checks living `readiness` checks readiness to receive traffic. They are critical for stateful apps, since their results provide ability to make right decision (restart the container or redirect the traffic) and explore (track) container (pod) state.

**Ordering Guarantee and Parallel Operations:**

Ordering guarantees are unnecessary for python-app, since it is not require some order in starting. Each pot is self-sufficient and does not require other pod for work. That is the reason, why we may realize `podManagementPolicy: Parallel`
...
```sh
(venv) vm@vm /m/v/d/h/V/U/L/P/D/S/k8s (lab14)> helm install cpp-app cpp-app
NAME: cpp-app
LAST DEPLOYED: Sun Mar 16 21:18:49 2025
NAMESPACE: default
STATUS: deployed
REVISION: 1
NOTES:
1. Get the application URL by running these commands:
  export POD_NAME=$(kubectl get pods --namespace default -l "app.kubernetes.io/name=cpp-app,app.kubernetes.io/instance=cpp-app" -o jsonpath="{.items[0].metadata.name}")
  export CONTAINER_PORT=$(kubectl get pod --namespace default $POD_NAME -o jsonpath="{.spec.containers[0].ports[0].containerPort}")
  echo "Visit http://127.0.0.1:8080 to use your application"
  kubectl --namespace default port-forward $POD_NAME 8080:$CONTAINER_PORT
(venv) vm@vm /m/v/d/h/V/U/L/P/D/S/k8s (lab14)> helm upgrade cpp-app ./cpp-app --set image.pullPolicy=Always
Release "cpp-app" has been upgraded. Happy Helming!
NAME: cpp-app
LAST DEPLOYED: Sun Mar 16 21:18:54 2025
NAMESPACE: default
STATUS: deployed
REVISION: 2
NOTES:
1. Get the application URL by running these commands:
  ...
  echo "Visit http://127.0.0.1:8080 to use your application"
  kubectl --namespace default port-forward $POD_NAME 8080:$CONTAINER_PORT
(venv) vm@vm /m/v/d/h/V/U/L/P/D/S/k8s (lab14)> kubectl get pods  -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.spec.containers[0].imagePullPolicy}{"\n"}{end}'
cpp-app-0	IfNotPresent
cpp-app-1	Always
cpp-app-2	Always
vault-0	IfNotPresent
vault-agent-injector-cb67b46-htkrl	IfNotPresent
```

`OnDelete` and `RollingUpdate` are `StatefulSet`'s update strategies. The first one updates pods after manual deleting, while the second updates them one by one from the last (optionally specified) automatically. Canary allows to update only part of containers (for testing new feature e .g). It was demonstrated above: only containers `2` and `1` were updated, since there was specified `partition: 1` and updates finished in the `1` instead of `0`. `Deployment` maintain strategies `RollingUpdate` and `Recreate`. `RollingUpdate` creates pods and deletes old after creation in random order and `Recreate` do some kind of vice verse (deletes all old pods and create new).
