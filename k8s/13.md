...
In this report you'll see the steps I followed to successfully complete lab 13, but spoiler, I didn't quite make it.

## Task 1: Deploy and Configure ArgoCD
### Task 1.1 Install ArgoCD via Helm
I followed the instructions for installing argocd as in the document you mentioned.
...
```
As you can see here, argocd has successfully deployed and is running.
...
Here I am already logged into admin account and have access to argocd panel.
![alt text](image-3.png)

### Task 1.4 Configure Python App Sync
I created the argocd folder and the argocd-python-app.yaml file. Updated the files to match the argocd work, ran `kubectl apply -f ArgoCD/argocd-python-app.yaml`. Then checked that everything ran correctly and as you can see from the output and screenshots below, everything works as it should.
```
...
### Task 1.5 Test Sync Workflow
Now I will update values.yaml replicaCount to 2. `replicaCount: 2`:
...
Conclusion: we have gone from installing argocd, to successfully deploying our web application.

## Task 2
### Task 2.1 Set Up Multi-Environment Configurations
The values-dev.yaml and values-prod.yaml files were created in our web-apps with different specific settings for them.

### Task 2.2 Create Namespaces
```
...
### Task 2.3 Deploy Multi-Environment via ArgoCD

```
lekski@LAPTOP-EA8M0FT5:/mnt/c/Users/Honor/Desktop/S25-core-course-labs/k8s$ kubectl apply -f ArgoCD/argocd-python-dev.yaml
...
As we can see argocd-python-dev.yaml deployed successfully, which can be seen on the argocd website.

![alt text](image-6.png)

### Task 2.4 
I had constant network problems throughout the whole mission. After starting Argocd it would work for about 10-20 minutes and then crash. Not only it crashed, but also the whole internet on my laptop and I had to restart it and start all over again. 
I have been trying to fix this problem for a long time. Starting from the fact that I decided that the problem is in the values.yaml and ingress configurations (which you can see in the commits in the github on the 13th branch.), and with my laptop. I tried many things, but nothing helped me to fix this error. There were several of them, you can see only one in the screenshot. 
Since argocd-python-prod.yaml takes longer to run, I never got it to run :(

![alt text](image-7.png)

## Bonus Task 

I also failed to run my bonus app in argocd, but I prepared all the necessary files for that and updated the configurations.
```
...
spec:
  project: default
  source:
    repoURL: https://github.com/lekski1/S25-core-course-labs.git
    targetRevision: lab13
    path: k8s/golang-web-app
    helm:
      valueFiles:
        - values.yaml
  destination:
