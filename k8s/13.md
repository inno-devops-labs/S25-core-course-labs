...
ArgoCD handles configuration drift by continuously comparing the desired state (from Git) with the live Kubernetes state. If differences are detected, it marks the application as OutOfSync and can automatically correct the drift if auto-sync is enabled. For runtime events (e.g., pod restarts, node failures), ArgoCD does not intervene directly, as these are managed by Kubernetes itself. However, it ensures that any unintended manual changes outside Git (e.g., kubectl edit) are reverted to maintain GitOps consistency.
