...
static void ev_handler(struct mg_connection *c, int ev, void *ev_data) {
    if (ev == MG_EV_HTTP_MSG) {
        ...
        if (mg_match(hm->uri, mg_str("/"), NULL)) {
            // If we are on the root, return the formatted page with the timestamp

            // Insert timestamp into template
            snprintf(page_render, sizeof(page_render), page_template, compute_time());
...
int main(void) {
    ...
    if (load_template(page_template, 200, "./resources/templates/template.html")) {
        return -1;
    }
