...
def test_response_contents(client):
    ...
    assert time_match is not None, "Time format is incorrect or MSK timezone not displayed"
...
def test_time_accuracy(client):
    ...
    response_time = moscow_tz.localize(datetime.strptime(response_time_str, '%Y-%m-%d %H:%M:%S'))
    test_time = datetime.now(moscow_tz)
    
    # Calculate time difference
    time_diff = abs((test_time - response_time).total_seconds())
    assert time_diff < 2, f"Time discrepancy too large: {time_diff} seconds"