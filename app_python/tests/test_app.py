...
class TestMoscowTimeApp(unittest.TestCase):
    ...
    def test_moscow_time_response_content(self):
        """Test that the response contains the expected 'Current time in Moscow' string."""
        response = self.app.get('/')
    ...
    def test_moscow_time_format(self):
        """Test that the response includes the time in the correct format (DD-MM-YYYY HH:MM:SS)."""
        response = self.app.get('/')
        ...
        try:
            ...
            self.fail(f"Time string '{time_str}' does not match the expected format '{expected_format}'")
    ...
    def test_moscow_time_accuracy(self):
        ...
        moscow_time_in_response_naive = datetime.strptime(time_str, '%d-%m-%Y %H:%M:%S')
        
        # Attach the Moscow timezone to the naive datetime object
        moscow_tz = pytz.timezone('Europe/Moscow')
        moscow_time_in_response = moscow_tz.localize(moscow_time_in_response_naive)
        ...
        # Compare the times
        time_difference = abs((current_moscow_time - moscow_time_in_response).total_seconds())
        self.assertLessEqual(time_difference, allowed_margin, 
                             f"Displayed time '{moscow_time_in_response}' does not match current Moscow time '{current_moscow_time}'")
