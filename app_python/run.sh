...
IMAGE_NAME="s25-devops-msk-time"
...
if [[ -z $(docker ps -a | grep -w "${IMAGE_NAME}_ct") ]]; then
    docker run -i \
        ...
        $([ "$DEV" == 1 ] && 
            ...
            echo "--mount type=bind,src=$SCRIPT_DIR/static,dst=/app/static";
            echo "--mount type=bind,src=$SCRIPT_DIR/templates,dst=/app/templates";
        ) \
        --env-file .env \
        -p "$APP_PORT:$APP_PORT" \
        --name "${IMAGE_NAME}_ct" \
